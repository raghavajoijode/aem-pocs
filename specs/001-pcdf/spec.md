...
**Independent Test**: Create one promotion in a region/country/locale folder, publish it, and confirm it is available for delivery when it is eligible.
...
A consumer application sends region, country, and locale plus optional context (brand, market, property, page type, promo, tag). The system returns exactly one winning promotion’s content or an explicit no-match result. Unrelated promotions are never returned.
...
2. **Given** the installed package, **When** a reviewer inspects identity, **Then** application nodes live under `/apps/aem-poc/pcdf`, promotions under `/content/dam/aem-poc/pcdf/{region}/{country}/{locale}`, models and configuration under `/conf/aem-poc-pcdf`, tags under `/content/cq:tags/pcdf`, and code uses `com.aem.poc.pcdf`.
...
Promotions live in region/country/locale folders and are not attached to individual pages. Many pages or consumer applications reuse the same promotion by requesting delivery with region, country, locale, and context.
...
1. **Given** a promotion authored only under a region/country/locale folder, **When** two different experiences request that folder with matching context, **Then** both can receive the same winning promotion.
...
- Request missing region, country, or locale: reject as invalid (do not invent defaults).
- Optional context fields omitted: those dimensions do not constrain matching.
- Empty targeting list on a promotion for a dimension: that dimension matches all request values. Empty brand tags mean match-all for `brand`.
- Promotion without the ACTIVE status tag (including INACTIVE): excluded even if dates and targeting match.
- Request region/country/locale whose DAM folder does not exist: explicit no-match (`contentFound` false); do not substitute another folder.
...
| Promotions | `/content/dam/aem-poc/pcdf/{region}/{country}/{locale}/…` |
...
- Membership tier, loyalty status, campaign audience, customer segment, device type, and other personalisation signals beyond the documented topology and targeting.
...
- Topology dimensions are region, country, and locale. Targeting dimensions in this proof of concept are start/end dates, market, property, page type, URL parameter (promo), brand tag, and topic tag.
